...
## Data
The data used is the [flower data set from kaggle](https://www.kaggle.com/olgabelitskaya/flower-color-images). To replicate the training, download the data set and create a training folder with gray photos and colored photos in different folder then adjust the pathing in the code. Since labeled pair is required, the order in which the images are loaded need to be the same for grayscale and colored photos. The input are also resized to 128x128. 
<br/>
